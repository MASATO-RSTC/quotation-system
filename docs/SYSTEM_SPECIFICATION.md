...
*   **PDF表示:**
    *   **明細テーブル:**
        *   `超過単価`: 割増係数適用後の最終的な金額のみが表示される。
        *   `控除単価`: 計算された控除単価が表示される。
        *   `深夜手当`, `法定休日出勤`などの各種割増単価が表示される。
    *   **特記事項:**
        *   `単価計算方法`の行に、計算タイプと差分時間が表示され、改行して超過・控除の計算式が表示される。
        *   例:
            ```
            ▼ 単価計算方法（出勤日数:20日 × 所定労働時間:8h 上限+8h 下限-8h）
            超過：500,000円 ÷ 168h
            控除：500,000円 ÷ 152h
            ```
...
*   **PDF表示:**
    ...
    *   **特記事項:**
        ...
        *   例:
            ```
            ▼ 単価計算方法（出勤日数:20日 × 所定労働時間:8h 上限+8h）
            超過：500,000円 ÷ 168h
            ```
...
*   **PDF表示:**
    ...
    *   **特記事項:**
        ...
        *   例:
            ```
            ▼ 単価計算方法（出勤日数:20日 × 所定労働時間:8h 下限-8h）
            控除：500,000円 ÷ 152h
            ```

#### 6.2.8. 月時（上限変動なし、下限変動なし）
*   **概要:** 月の労働日数と1日の労働時間から基準時間（＝上限時間・下限時間）を算出する契約形態。差分の入力はない。
*   **追加入力項目:**
    *   `労働日数/月`: 基準時間計算のための月の労働日数。
    *   `労働時間/日`: 基準時間計算のための1日の労働時間。
    *   `計算タイプ`: 特記事項に表示するための計算式の種類を選択する。（計算結果には影響しない）
    *   `超過単価の算出基準`: 超過単価を計算する際の基準時間を選択する。
    *   `控除単価の算出基準`: 控除単価を計算する際の基準時間を選択する。
    *   `割増係数`: 超過単価にのみ適用される割増の係数。
    *   `各種割増率`: 深夜、休日出勤などの割増率。
*   **計算ロジック:**
    *   上限時間・下限時間（基準時間） = `労働日数/月` × `労働時間/日`
    *   超過・控除単価は、上記で計算された上限・下限時間と、既存の「月時（上限あり下限あり）」と同様のロジックで計算される。
*   **PDF表示:**
    *   **明細テーブル:**
        *   `時間幅`: `下限時間h 〜 上限時間h` の形式で表示される。
        ...
        *   各種割増単価が表示される。
    *   **特記事項:**
        *   `単価計算方法`の行に、計算タイプが表示され、改行して超過・控除の計算式が表示される。
        *   例:
            ```
            ▼ 単価計算方法（出勤日数:20日 × 所定労働時間:8h）
            超過：500,000円 ÷ 160h
            控除：500,000円 ÷ 160h
            ```